...
public class Category extends AbstractModel {
	...
	public boolean save() {
		...
		try{
			...
			if(isNew()) prepareStatement.setString(3, new SimpleDateFormat("yyyy-MM-dd HH:MM:SS").format(new Date()).toString());
			else prepareStatement.setString(3, new SimpleDateFormat("yyyy-MM-dd HH:MM:SS").format(createAt).toString());			
			prepareStatement.setString(3, new SimpleDateFormat("yyyy-MM-dd HH:MM:SS").format(createAt).toString());
			prepareStatement.setString(4, new SimpleDateFormat("yyyy-MM-dd HH:MM:SS").format(new Date()).toString());
			prepareStatement.addBatch();
			boolean result = prepareStatement.executeBatch().length > 0;
			if(result && isNew()) afterSave(); return true;
		}catch(Exception e){e.printStackTrace(); return false;}
	}
	protected void makeTransaction(String sql, int bookId){
		try{			
			PreparedStatement prepareStatement = AbstractConnection.getConnection().executePrepareStatement(sql);
	...
	public void addBook(Book book){
		makeTransaction("insert into books_categories(category_id, book_id) values(?, ?);", book.getId());
	}
	public void removeBook(Book book){
		makeTransaction("delete from books_categories where category_id= ? AND book_id= ?;", book.getId());
	}
	@Override
